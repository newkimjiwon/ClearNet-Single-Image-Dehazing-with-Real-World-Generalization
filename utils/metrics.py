...
def ssim(img1, img2, window_size=11, window=None, size_average=True, full=False, val_range=None):
    """Calculates SSIM index between two images"""
...
def batch_psnr(img, imclean, data_range=1.0):
    ...
    imclean = imclean.cpu().detach().numpy().astype(np.float32)
    
    batch_psnr = 0
    for i in range(img.shape[0]):
        batch_psnr += psnr_numpy(img[i], imclean[i], data_range)
    return batch_psnr / img.shape[0]


def psnr_numpy(img1, img2, data_range=1.0):
    """Calculates PSNR between two numpy images"""
...
class SSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        if channel == self.window.shape[0]:
            window = self.window
        else:
            window = create_window(self.window_size, channel).to(img1.device)
            self.window = window

        return ssim(img1, img2, window_size=self.window_size, window=window, 
